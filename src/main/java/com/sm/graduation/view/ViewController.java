...
public class ViewController {
    ...
    /** 为修改事故信息进行的通过id对数据进行查询 */
    @GetMapping("/modifyAcc")
    public String modifyAcc(@RequestParam(value = "id") long id,
                            Model model){
        AccidentPojo accidentPojo = accidentService.listById(id);
        model.addAttribute("a",accidentPojo);
...
//   出院
    ...
    public String discharge(){
        return "/page/discharge";
    }

//    添加的页面操作
    @GetMapping("/addis")
    public String addis(){
        return "/page/add/addis";
    }
    ...
    //编辑
    @GetMapping("/modifydis")
    public String modifydis(@RequestParam(value = "id") Integer id,Model model){
//        通过id对数据进行查询
        UserInfomation userInfomation = userInformationService.selectById(id);
        model.addAttribute("userInformationuu",userInfomation);
//        查询user数据信息
        List<UserReq> userReqs = userInformationMapper.selectByUser();
        model.addAttribute("user",userReqs);
//        查询药品信息
        List<UserReq> userReqs1 = userInformationMapper.selectBYmedication();
        model.addAttribute("medication",userReqs1);
        return "page/modify/modifyis";
    }

//    查询
//    可视化页面跳转
    @GetMapping("/zhanshi")
public String zhanshi(){ return "/page/zhanshi"; }
